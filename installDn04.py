...
";
dn04InitParam="initdb --locale=en_US.UTF-8 -U tbase -E utf8 -D /data/tbase/dn04 --nodename=dn04 --nodetype=datanode --master_gtm_nodename gtm_1 --master_gtm_ip "+ip+" --master_gtm_port 21000";
updateDNO2Param = "\
\n \
port = 23004 \n  \
pooler_port = 24001  \n \
...
host    replication     all             "+ip.rsplit(".",1)[0]+".0/24           trust  \n \
";
startParam="pg_ctl -Z datanode start -D /data/tbase/dn04 \
";
def baseConfigInit ():
    print("-----开始安装tbase----")
...
def updatedn04ConfigInit():
    startGtmResult = os.system(dn04InitParam)
    print("启动" + str(startGtmResult))
    createFile("/data/tbase/dn04/postgresql.conf",updateDNO2Param)
    createFile("/data/tbase/dn04/pg_hba.conf", updateDNO2Pghba)
